...
namespace Syncfusion.Maui.Toolkit.UnitTest
{
    public class InterfaceUnitTests:BaseUnitTest
    {
        ...
        public void GetTooltipInfo_ValidInput_ReturnsTooltipInfo()
        {
            var sfCartesianChart = new SfCartesianChart();
            ColumnSeries columnSeries = new ColumnSeries();
            sfCartesianChart.Series.Add(columnSeries);
            ...
            float x = 10.0f;
            float y = 20.0f;
            var result = ((IChart)sfCartesianChart).GetTooltipInfo(chartTooltipBehavior, x, y);
        ...
        [Fact(Skip ="need to check later")]
        public void SetTooltipTargetRect_SetsCorrectTargetRect()
        {
            var source = new object();
            var tooltipInfo = new TooltipInfo(source);
            var chartBounds = new Rect(0, 0, 100, 100);
            ((ITooltipDependent)tooltipInfo.Source).SetTooltipTargetRect(tooltipInfo, chartBounds);
            Assert.Equal(new Rect(9.5, 19.5, 1, 1), tooltipInfo.TargetRect);
        }
        ...
        public void GetMaximumSizeCoefficient_ReturnsDefaultSize()
        {
            ...
            var chartLegend = new ChartLegend();
            var result = InvokePrivateMethod(chartLegend, "GetMaximumSizeCoefficient", Array.Empty<object>());
        ...
        {
            var chartLegend = new ChartLegend();
            var exception = Record.Exception(() => InvokePrivateMethod(chartLegend, "GetMaximumSizeCoefficient", Array.Empty<object>()));
