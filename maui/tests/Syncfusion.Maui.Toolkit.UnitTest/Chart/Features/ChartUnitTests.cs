...
namespace Syncfusion.Maui.Toolkit.UnitTest
{
    public class ChartUnitTests:BaseUnitTest
    {
        ...
        public void GetTooltipInfo_ShouldReturnTooltip_Polar()
        {
            var sfPolarChart = new SfPolarChart();
             sfPolarChart.SeriesBounds = new Rect(0, -1, 1007.599975,735.20001220703125);
            var chartPolar = new PolarChartArea(sfPolarChart);
            chartPolar.PolarAxisCenter = new Point(518.8f, 252.4f);
            PolarLineSeries polarLineSeries = new PolarLineSeries();
            polarLineSeries.ActualXAxis= new NumericalAxis() { AvailableSize=new Size(1037.5999755859375,504.79998779296875) };
            polarLineSeries.ActualYAxis= new NumericalAxis() {  RenderedRect=new Rect( 488.8f, 42f,   548.8f,  210.4f) };
            sfPolarChart.PrimaryAxis = new NumericalAxis() { AvailableSize = new Size(1037.5999755859375, 504.79998779296875) };
            sfPolarChart.SecondaryAxis = new NumericalAxis() { RenderedRect = new Rect(488.8f, 42f, 548.8f, 210.4f) };
            polarLineSeries.ActualXValues = new List<string>() { "ram","ravi" };
            polarLineSeries.YValues = new List<double>() { 10 ,30};
            polarLineSeries.EnableTooltip = true;
            polarLineSeries.PointsCount = 1;
            sfPolarChart.Series.Add(polarLineSeries);
            IChart series = (IChart)sfPolarChart;
            series.ActualSeriesClipRect = sfPolarChart.SeriesBounds;
            ChartTooltipBehavior tooltip = new ChartTooltipBehavior();
            sfPolarChart.TooltipBehavior = tooltip;
            ObservableCollection<ChartSegment> segments = new ObservableCollection<ChartSegment>() { new PolarLineSegment()};
            SetPrivateField(polarLineSeries, "Segments", segments);
            var result = sfPolarChart.GetTooltipInfo(tooltip, 518.4f, 258.4f);
            Assert.IsType<TooltipInfo>(result); 
        }
        ...
        [Fact(Skip = "need to check")]
        public void GetDataType_ReturnsString_WhenInputIsString()
        {
            var sfPyramidChart = new SfPyramidChart();
            var input = "test string";
            var result = InvokePrivateMethod(sfPyramidChart, "GetDataType", new object[] { input });
            Assert.Equal(ChartValueType.String, result);
        }

        [Fact(Skip = "need to check")]
        public void GetDataType_ReturnsDateTime_WhenInputIsDateTime()
        {
            var sfPyramidChart = new SfPyramidChart();
            var input = DateTime.Now;
            var result = InvokePrivateMethod(sfPyramidChart, "GetDataType", new object[] { input });
            Assert.Equal(ChartValueType.DateTime, result);
        }

        [Fact(Skip = "need to check")]
        public void GetDataType_ReturnsDouble_WhenInputIsInteger()
        {
            var sfPyramidChart = new SfPyramidChart();
            var input = 42;
            var result = InvokePrivateMethod(sfPyramidChart, "GetDataType", new object[] { input });
            Assert.Equal(ChartValueType.Double, result);
        }
        ...
        [Fact(Skip ="need to check")]
        public void AnimateSeries_Circular()
        {
            var sfPyramidChart = new SfCircularChart();
            var chartCircular = new CircularChartArea(sfPyramidChart);
            ...
            sfPyramidChart.Series.Add(pieSeries);
            sfPyramidChart.AnimateSeries();
        }
    }
}
