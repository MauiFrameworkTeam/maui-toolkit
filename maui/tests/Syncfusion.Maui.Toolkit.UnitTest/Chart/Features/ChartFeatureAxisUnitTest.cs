...
namespace Syncfusion.Maui.Toolkit.UnitTest
{
    public class ChartFeatureAxisUnitTest : BaseUnitTest
    {
        ...
        public void RaiseActualRangeChangedEvent_ShouldUpdateActualRange_WhenEventTriggered()
        {
            var axis = new NumericalAxis();
            DoubleRange visibleRange = new DoubleRange(0, 1);
            Size plotSize = new Size(100, 100);
            axis.ActualRangeChanged += (sender, args) =>
            {
                args.Axis = axis;
            };

            InvokePrivateMethod(axis, "RaiseActualRangeChangedEvent", new object[] { visibleRange, plotSize });

            Assert.Equal(visibleRange.Start, axis.VisibleRange.Start);
            Assert.Equal(visibleRange.End, axis.VisibleRange.End);
        }
        ...
        [Theory (Skip ="Failed case")]
        [InlineData(0, 100, 10, 0, 100)]  
        [InlineData(0, 100, 10, 5, 105)]  
        public void ApplyRangePadding_ShouldApplyPaddingCorrectly(double start, double end, double interval, double expectedStart, double expectedEnd)
        {
            var axis = new DateTimeAxis();
            var range = new DoubleRange(start, end);
        ...
        [Fact (Skip ="Failed case")]
        public void GenerateVisibleLabels_ShouldGenerateCorrectLabels_WhenVisibleRangeIsValid()
        {
            var axis = new DateTimeAxis();
            axis.VisibleRange = new DoubleRange(DateTime.Now.AddYears(-1).ToOADate(), DateTime.Now.ToOADate());
            axis.VisibleInterval = 1;

            axis.GenerateVisibleLabels();

            Assert.NotEmpty(axis.VisibleLabels);
            Assert.True(axis.TickPositions.Count > 0);
        }
        ...
        [Theory (Skip = "Failed case")]
        [InlineData(DateTimeRangePadding.Round, "2020-01-01", "2022-01-01", "2019-12-01", "2022-12-01", "2020-01-01", "2022-01-01", "2019-12-01", "2022-12-01")]
        [InlineData(DateTimeRangePadding.RoundStart, "2020-01-01", "2022-01-01", "2019-12-01", "2022-12-01", "2020-01-01", "2022-01-01", "2019-12-01", "2022-01-01")]
        [InlineData(DateTimeRangePadding.RoundEnd, "2020-01-01", "2022-01-01", "2019-12-01", "2022-12-01", "2020-01-01", "2022-01-01", "2020-01-01", "2022-12-01")]
        [InlineData(DateTimeRangePadding.PrependInterval, "2020-01-01", "2022-01-01", "2019-12-01", "2022-12-01", "2020-01-01", "2022-01-01", "2019-12-01", "2022-01-01")]
        [InlineData(DateTimeRangePadding.AppendInterval, "2020-01-01", "2022-01-01", "2019-12-01", "2022-12-01", "2020-01-01", "2022-01-01", "2020-01-01", "2022-12-01")]
        [InlineData(DateTimeRangePadding.Additional, "2020-01-01", "2022-01-01", "2019-12-01", "2022-12-01", "2020-01-01", "2022-01-01", "2019-12-01", "2022-12-01")]
        [InlineData(DateTimeRangePadding.None, "2020-01-01", "2022-01-01", "2019-12-01", "2022-12-01", "2020-01-01", "2022-01-01", "2020-01-01", "2022-01-01")]
        public void ApplyRangePadding_Test(DateTimeRangePadding rangePadding, string additionalStart, string additionalEnd, string roundStart, string roundEnd, string start, string end, string expectedStart, string expectedEnd)
        {
            var axis = new DateTimeAxis();
            axis.RangePadding = rangePadding;

            DateTime additionalStartDate = DateTime.Parse(additionalStart);
            DateTime additionalEndDate = DateTime.Parse(additionalEnd);
            DateTime roundStartDate = DateTime.Parse(roundStart);
            DateTime roundEndDate = DateTime.Parse(roundEnd);
            DateTime startDate = DateTime.Parse(start);
            DateTime endDate = DateTime.Parse(end);

            var result = InvokePrivateMethod(axis, "ApplyRangePadding", new object[] { additionalStartDate, additionalEndDate, roundStartDate, roundEndDate, startDate, endDate} );

            Assert.NotNull(result);
            Assert.Equal(DateTime.Parse(expectedStart).ToOADate(), ((DoubleRange)result).Start);
            Assert.Equal(DateTime.Parse(expectedEnd).ToOADate(), ((DoubleRange)result).End);
        }
        [Theory]
